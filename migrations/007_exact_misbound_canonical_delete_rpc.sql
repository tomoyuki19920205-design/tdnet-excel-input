-- Atomically delete a bounded set of redundant, historically misbound
-- segment_canonical rows.  Every mutable and provenance value of both the
-- delete target and its already-existing canonical destination is an exact
-- guard.  No insert/update path exists in this function.
...
BEGIN
    ...
    IF EXISTS (
        ...
        WHERE jsonb_typeof(item) <> 'object'
           ...
           OR NULLIF(btrim(item->>'misbound_segment_key'), '') IS NULL
           ...
           OR NULLIF(btrim(item->>'destination_segment_key'), '') IS NULL
