-- Atomic, exact-predicate repair for a bounded set of segment_canonical sales rows.
-- The caller-supplied segment_id is an immutable audit correlation ID.  The
-- remote table has no segment ID column, so the row lock uses its complete PK.
CREATE OR REPLACE FUNCTION public.repair_segment_canonical_sales_exact(p_repairs jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    IF v_requested = 0 OR v_requested > 10 THEN
        RAISE EXCEPTION 'repair payload count must be between 1 and 10' USING ERRCODE = '22023';
    END IF;
    IF EXISTS (
        SELECT 1 FROM jsonb_array_elements(p_repairs) AS item
        WHERE jsonb_typeof(item) <> 'object'
           OR NOT item ?& ARRAY[
               'segment_id','existing_sales','replacement_sales','existing_profit',
           ...
           OR NULLIF(btrim(item->>'segment_key'), '') IS NULL
           ...
           OR (item->>'existing_sales')::bigint = (item->>'replacement_sales')::bigint
    ) THEN
    ...
    -- Lock every destination in a deterministic order before updating any row.
    FOR v_item IN
    ...
    LOOP
        ...
        FOR UPDATE;
        IF NOT FOUND THEN
            RAISE EXCEPTION 'repair target missing for segment_id %', v_item->>'segment_id'
                USING ERRCODE = 'P0002';
        END IF;
        v_locked := v_locked + 1;
        IF v_row.sales IS DISTINCT FROM (v_item->>'existing_sales')::bigint
           ...
           OR v_row.updated_at IS DISTINCT FROM (v_item->>'existing_updated_at')::timestamptz THEN
            RAISE EXCEPTION 'exact before predicate mismatch for segment_id %', v_item->>'segment_id'
                USING ERRCODE = 'P0001';
        END IF;
    ...
    LOOP
        ...
        IF NOT FOUND THEN
            RAISE EXCEPTION 'guarded update failed for segment_id %', v_item->>'segment_id'
                USING ERRCODE = 'P0001';
        END IF;
        ...
        IF v_row.sales IS DISTINCT FROM (v_item->>'replacement_sales')::bigint
           ...
           OR v_row.source IS DISTINCT FROM v_item->>'source'
           OR v_row.updated_at IS DISTINCT FROM (v_item->>'existing_updated_at')::timestamptz THEN
            RAISE EXCEPTION 'post-update verification failed for segment_id %', v_item->>'segment_id'
                USING ERRCODE = 'P0001';
        END IF;
        v_result_rows := v_result_rows || jsonb_build_array(jsonb_build_object(
            ...
            'segment_name', v_row.segment_name,
...
REVOKE ALL ON FUNCTION public.repair_segment_canonical_sales_exact(jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.repair_segment_canonical_sales_exact(jsonb) FROM anon;
REVOKE ALL ON FUNCTION public.repair_segment_canonical_sales_exact(jsonb) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.repair_segment_canonical_sales_exact(jsonb) TO service_role;
