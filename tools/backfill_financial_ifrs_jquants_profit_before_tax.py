#!/usr/bin/env python3
"""Exact-six-ticker IFRS actual PBT manifest and production apply tool."""
...
from src.jquants.financial_details import (
    actual_consolidated_document_info,
    select_latest_effective_pbt,
)
from tools.backfill_structural_no_op_jquants_profit_before_tax import (
    ENDPOINT,
    OFFICIAL_SOURCES,
    SUMMARY_ENDPOINT,
    VALID_QUARTERS,
...
COMPANY_CONFIG: dict[str, dict[str, str]] = {
    "7198": {"name": "SBIアルヒ", "jquants_code": "71980"},
    "8473": {"name": "SBIホールディングス", "jquants_code": "84730"},
    "8698": {"name": "マネックスグループ", "jquants_code": "86980"},
    "8253": {"name": "クレディセゾン", "jquants_code": "82530"},
    "7157": {"name": "ライフネット生命保険", "jquants_code": "71570"},
    "8630": {"name": "SOMPOホールディングス", "jquants_code": "86300"},
}
TICKERS = tuple(COMPANY_CONFIG)
METRIC = "profit_before_tax"
SOURCE = "jquants"
APPLY_TOKEN = "I_UNDERSTAND_FINANCIAL_IFRS_SIX_JQUANTS_PBT_WRITE"
FORECAST_SOURCES = {
    "jquants_nxf",
    "jquants_forecast_fy",
...
def select_latest_actual_statement_metadata(
    details: list[dict[str, Any]],
    summaries: dict[str, dict[str, Any]],
    *,
    expected_code: str,
) -> dict[tuple[str, str], dict[str, str]]:
    """Select latest actual consolidated statement before inspecting PBT."""
    selected: dict[tuple[str, str], tuple[tuple[str, str, str], dict[str, str]]] = {}
    for detail in details:
        disc_no = str(detail.get("DiscNo") or "")
        summary = summaries.get(disc_no)
        if detail.get("Code") != expected_code or summary is None:
            continue
        if summary.get("Code") != expected_code or detail.get("DocType") != summary.get("DocType"):
            continue
        document_type = str(detail.get("DocType") or "")
        info = actual_consolidated_document_info(document_type)
        if info is None or summary.get("CurPerType") != info[0] or not summary.get("CurFYEn"):
            continue
        ...
        metadata = {
            "accounting_standard": info[1],
            "document_type": document_type,
            "disclosure_number": disc_no,
        }
        previous = selected.get(key)
        if previous is None or order > previous[0]:
            selected[key] = (order, metadata)
...
def _company_manifest_rows(
    ...
    statement_by_period = select_latest_actual_statement_metadata(
        source_data["details"], source_data["summaries"], expected_code=code
    )
    ...
    for item in canonical:
        ...
        if item.get("source") in FORECAST_SOURCES:
            continue
        if (
            item.get("source") == "tdnet"
            and item.get("filing_id") is None
            and item.get("disclosure_datetime") is None
        ):
            continue
    ...
    for period, quarter in sorted(viewer_periods):
        key = (period, quarter)
        existing = canonical_by_period.get(key, [])
        official = [row for row in existing if row.get("source") in OFFICIAL_SOURCES]
        statement = statement_by_period.get(key)
        standard = statement.get("accounting_standard") if statement else None
        record = valid_by_period.get(key)
        if standard != "IFRS":
            action = "NO_ACTION_NON_IFRS_PERIOD" if standard else "NO_ACTION_NO_VALID_PBT"
        elif official:
            action = "NO_ACTION_OFFICIAL_PBT_EXISTS"
        elif existing:
            action = "NO_ACTION_NO_VALID_PBT"
        elif record is not None and record.accounting_standard == "IFRS":
        ...
        row: dict[str, Any] = {
            ...
            "source": SOURCE if record and record.accounting_standard == "IFRS" else None,
        ...
        rows.append(row)
    return rows, extraction_audit, sum(record.accounting_standard == "IFRS" for record in records)
...
def build_manifest(
    ...
    manifest: dict[str, Any] = {
        ...
        "valid_ifrs_jquants_period_count_by_ticker": valid_counts,
        "expected_insert_count_by_ticker": {
            ticker: sum(row["ticker"] == ticker for row in inserts) for ticker in TICKERS
        },
        "non_ifrs_period_count_by_ticker": {
            ticker: sum(
                row["ticker"] == ticker and row["intended_action"] == "NO_ACTION_NON_IFRS_PERIOD"
                for row in rows
            )
            for ticker in TICKERS
        },
...
def apply_manifest(
    ...
    *,
    ...
    if any(
        row.get("ticker") not in TICKERS
        or row.get("source") != SOURCE
        or row.get("accounting_standard") != "IFRS"
        for row in targets
    ):
        raise RuntimeError("out-of-scope or non-IFRS manifest row")
    ...
    if not result.get("ok") or result.get("count") != expected_count:
        raise RuntimeError(f"financial IFRS PBT insert failed: {result}")
    after = {
