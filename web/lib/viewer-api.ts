...
/**
 * edinet_order_data テーブルから過去年度の受注KPIを取得する。
 */
export async function loadEdinetOrders(
  supabase: SupabaseClient,
  ticker: string,
): Promise<EdinetOrderRecord[]> {
  const t = normalizeTicker(ticker);
  if (!t) return [];
  ...
  try {
    ...
    if (error) {
      console.warn("[viewer-api] edinet_order_data 取得エラー:", error.message);
      return [];
    }

    // NULLしかない行は非表示とする（orders_receivedまたはorder_backlogが存在する行のみ）
    const validData = (data as EdinetOrderRecord[] || []).filter(
      r => r.orders_received != null || r.order_backlog != null
    );

    return validData;
  } catch (err) {
    console.warn("[viewer-api] edinet_order_data 例外:", err);
    return [];
  }
}
